...
import { Upload, Users, CheckCircle, AlertCircle, XCircle, ChevronDown, X } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { getAuthHeaders } from "@/lib/auth";

interface VcfImportResults {
  imported: number;
...
export function ContactImportPage() {
  ...
  const [results, setResults] = useState<VcfImportResults | null>(null);
  ...
  const importMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      const response = await fetch("/api/import-contacts-vcf", {
        method: "POST",
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          throw new Error("Please sign in to import contacts");
        }
        throw new Error(error.details || error.error || "Import failed");
      }

      const data = await response.json();
      return data.results as VcfImportResults;
    },
    onSuccess: (data) => {
      setResults(data);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (droppedFile) {
      ...
      const isValidFile = fileName.endsWith(".vcf");
      ...
      if (isValidFile) {
        ...
        toast({
          ...
          description: "Please upload a VCF file (.vcf)",
        });
      }
    }
  };
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 -mt-0.5">Import contacts from VCF (vCard) file</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <CardContent className="space-y-6 pt-6">
              ...
              {file ? (
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-gray-500 mt-0.5">VCF (.vcf) export from iPhone, Android, or contacts app</div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-end">
              <Button
                ...
                size="lg"
                className="min-w-[180px] font-semibold bg-blue-600 hover:bg-blue-700"
              >
            ...
            <CardContent className="space-y-6 pt-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">VCF import finished</p>
                </div>
              </div>
            ...
            <div className="border-t px-4 pb-4 pt-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="rounded-lg border p-4">
                  ...
                  <p className="text-sm text-gray-600">Phone numbers are normalized to E.164 format. Company names are extracted and deduplicated.</p>
                </div>
                <div className="rounded-lg border p-4">
                  <div className="font-medium text-sm text-gray-900 mb-2">Duplicate Detection</div>
                  <p className="text-sm text-gray-600">Contacts with the same name, company, and phone/email are deduplicated automatically.</p>
                </div>
                <div className="rounded-lg border p-4">
                  <div className="font-medium text-sm text-gray-900 mb-2">Company Linking</div>
                  <p className="text-sm text-gray-600">Contacts are linked to companies. Companies are created when missing and reused when matching.</p>
                </div>
              </div>
            </div>
          )}
